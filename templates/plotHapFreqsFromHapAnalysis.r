...
combineHapFiles <- function(files, hap)
{
    ...
    for (file in files) 
    {
        ...
        print(paste0("processed ", file_name, "..."), quote=F) # REMOVE ME
...
{
    transposedHap <- read.csv2("${params.outPrefix}-transpoded-hap-matrix.csv", h=T, quote="", sep=",")
...
{
    pdf("${params.outPrefix}-hap.pdf", colormodel='cmyk')
